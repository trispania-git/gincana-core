...
add_shortcode('gincana_estaciones_lista', function($atts){
  ...
  $qr_needs_scan        = in_array($tipo_qr, ['validacion_boton', 'validacion_quiz', 'validacion'], true);
  ...
  <div id="<?php echo esc_attr($uid); ?>" style="width:95%;max-width:760px;margin:0 auto;">
    ...
    <?php if ($qr_needs_scan && $user_id && $completed < $total): ?>
    <!-- Botón escanear QR -->
    <div style="text-align:center;margin-bottom:20px;">
      <button type="button" id="gc-qr-scan-btn-<?php echo $uid; ?>" style="display:inline-flex;align-items:center;gap:10px;padding:14px 28px;border:0;border-radius:14px;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:#fff;font-size:17px;font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(37,99,235,0.35);transition:transform 0.2s,box-shadow 0.2s;">
    ...
    <!-- Modal escáner QR -->
    <div id="gc-qr-modal-<?php echo $uid; ?>" style="display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.85);flex-direction:column;align-items:center;justify-content:center;">
      <div style="position:relative;width:100%;max-width:480px;padding:16px;">
        ...
        <div style="position:relative;border-radius:16px;overflow:hidden;background:#000;aspect-ratio:1/1;">
          <video id="gc-qr-video-<?php echo $uid; ?>" autoplay playsinline muted style="width:100%;height:100%;object-fit:cover;"></video>
          <!-- Marco de enfoque -->
          <div style="position:absolute;inset:15%;border:3px solid rgba(255,255,255,0.7);border-radius:12px;pointer-events:none;"></div>
        </div>
        <canvas id="gc-qr-canvas-<?php echo $uid; ?>" style="display:none;"></canvas>
        <div id="gc-qr-status-<?php echo $uid; ?>" style="text-align:center;margin-top:12px;color:#94a3b8;font-size:14px;">Apunta la camara al codigo QR...</div>
        <button type="button" id="gc-qr-close-<?php echo $uid; ?>" style="display:block;margin:16px auto 0;padding:12px 32px;border:2px solid rgba(255,255,255,0.4);border-radius:12px;background:transparent;color:#fff;font-size:16px;font-weight:600;cursor:pointer;">
          Cerrar
        </button>
      </div>
    </div>
    ...
    (function(){
      ...
      var btn   = document.getElementById('gc-qr-scan-btn-' + uid);
      var modal = document.getElementById('gc-qr-modal-' + uid);
      var video = document.getElementById('gc-qr-video-' + uid);
      var canvas = document.getElementById('gc-qr-canvas-' + uid);
      var status = document.getElementById('gc-qr-status-' + uid);
      var closeBtn = document.getElementById('gc-qr-close-' + uid);
      var ctx = canvas.getContext('2d', {willReadFrequently: true});
      var stream = null;
      var scanning = false;
      var scanInterval = null;

      // URL base que esperamos (token QR de este sitio)
